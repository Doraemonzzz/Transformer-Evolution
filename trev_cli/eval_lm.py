...
import torch
import trev
from trev import checkpoint_utils, distributed_utils, options, tasks, utils
from trev.dataclass.utils import convert_namespace_to_omegaconf
from trev.logging import progress_bar
from trev.logging.meters import StopwatchMeter
...
logger = logging.getLogger("trev_cli.eval_lm")


def eval_lm(
    models: List[trev.models.TrevModel],
    source_dictionary: trev.data.Dictionary,
    batch_iterator: Iterable,
    post_process: Optional[str] = None,
    output_word_probs: bool = False,
    output_word_stats: bool = False,
    target_dictionary: Optional[trev.data.Dictionary] = None,
    ...
    """
    Args:
        models (List[~trev.models.TrevModel]): list of models to
            evaluate. Models are essentially `nn.Module` instances, but
            must be compatible with trev's `SequenceScorer`.
        source_dictionary (~trev.data.Dictionary): dictionary for
        ...
        post_process (Optional[str]): post-process text by removing BPE,
            ...
            trev.data.utils.post_process, although not all options
        ...
        target_dictionary (Optional[~trev.data.Dictionary]): output

...
from trev.modules import Urpe
from trev.modules import UrpeV2
from trev.modules import GLU
from trev.modules import ConvMix
from trev.modules import OffsetScale
from einops import rearrange

@with_incremental_state
class GauQuad(nn.Module):
    ...
    def __init__(
        ...
        d1 = int(7 * embed_dim / 4)
        d2 = int(3 * embed_dim / 4)
        d2 = (d2 // num_heads) * num_heads
        self.head_dim = d2 // num_heads
        self.q_offsetscale = OffsetScale(d2)
        self.k_offsetscale = OffsetScale(d2)
        # d^2
        self.base_proj = quant_noise(
        ...
        if self.attention_use_layer_norm:
            self.layer_norm = self.get_norm_fun(self.norm_type, d1)
        ...
        if use_final_dropout:
            self.final_dropout_module = TrevDropout(
    ...
    def gau_init(self):
        ...
        nn.init.normal_(self.base_proj.bias, std=0.02)

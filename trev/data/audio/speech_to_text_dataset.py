...
from trev.data import (
    ...
    TrevDataset,
    ResamplingDataset,
    data_utils as trev_data_utils,
)
from trev.data.audio.audio_utils import get_fbank, get_waveform
from trev.data.audio.feature_transforms import CompositeAudioFeatureTransform
...
class S2TDatacosnfig(object):
    ...
    def vocab_filename(self):
        """trev vocabulary file under data root"""
    ...
    def pre_tokenizer(self) -> Dict:
        ...
        Tokenizers are defined in `trev.data.encoders.*`"""
...
class SpeechToTextDataset(TrevDataset):
    ...
    def collater(self, samples: List[Tuple[int, torch.Tensor, torch.Tensor]]) -> Dict:
        ...
        ntokens = None
        if self.tgt_texts is not None:
            target = trev_data_utils.collate_tokens(
            ...
            prev_output_tokens = trev_data_utils.collate_tokens(
                [t for _, _, t in samples],
                self.tgt_dict.pad(),
                self.tgt_dict.eos(),
                left_pad=False,

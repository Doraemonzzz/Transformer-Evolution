...
import torch
from trev import checkpoint_utils, models, optim, utils
from trev.dataclass.configs import TrevConfig
from trev.dataclass.utils import convert_namespace_to_omegaconf
from trev.distributed import utils as distributed_utils
from trev.file_io import PathManager
from trev.logging import meters, metrics
from trev.nan_detector import NanDetector
from trev.optim import lr_scheduler
...
class Trainer(object):
    ...
    def __init__(self, cfg: TrevConfig, task, model, criterion, quantizer=None):
        ...
        if (
            ...
            # the DistributedTrevModel wrapper will handle moving to device,
    ...
    def criterion(self):
        if self._wrapped_criterion is None:
            if (
                ...
                self._wrapped_criterion = models.DistributedTrevModel(
    ...
    def model(self):
        if self._wrapped_model is None:
            if self.use_distributed_wrapper:
                self._wrapped_model = models.DistributedTrevModel(
                    self.cfg.distributed_training,
                    self._model,
                    process_group=self.data_parallel_process_group,
                    device=self.device,
    ...
    def _build_optimizer(self):
        ...
        if self.cfg.optimization.use_bmuf:
            self._optimizer = optim.TrevBMUF(
                self.cfg.bmuf,
                self._optimizer,
            )
    ...
    def get_meter(self, name):
        ...
        from trev import meters

        if "get_meter" not in self._warn_once:
            self._warn_once.add("get_meter")
            utils.deprecation_warning(
                "Trainer.get_meter is deprecated. Please use trev.metrics instead."
            )
    ...
    def _xla_markstep_and_send_to_cpu(self, data=None):
        ...
        xm.mark_step()
        if data is not None:
            from trev.utils import xla_device_to_cpu

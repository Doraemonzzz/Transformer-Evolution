...
def hydra_init(cfg_name="config") -> None:
    ...
    cs.store(name=cfg_name, node=TrevConfig)

    for k in TrevConfig.__dataclass_fields__:
        v = TrevConfig.__dataclass_fields__[k].default
        try:
...
def add_defaults(cfg: DictConfig) -> None:
    ...
    from trev.models import ARCH_MODEL_NAME_REGISTRY, MODEL_DATACLASS_REGISTRY
    from trev.dataclass.utils import merge_with_parent
    from typing import Any

    OmegaConf.set_struct(cfg, False)

    for k, v in TrevConfig.__dataclass_fields__.items():
        field_cfg = cfg.get(k)

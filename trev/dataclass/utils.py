...
from trev.dataclass.configs import TrevConfig
...
def gen_parser_from_dataclass(
    ...
    dataclass_instance: TrevDataclass,
    ...
    def get_kwargs_from_dc(
        dataclass_instance: TrevDataclass, k: str
    ) -> Dict[str, Any]:
    ...
    for k in dataclass_instance._get_all_attributes():
        ...
        if field_name is None:
            continue
        elif inspect.isclass(field_type) and issubclass(field_type, TrevDataclass):
...
def _override_attr(
    sub_node: str, data_class: Type[TrevDataclass], args: Namespace
) -> List[str]:
    overrides = []

    if not inspect.isclass(data_class) or not issubclass(data_class, TrevDataclass):
        return overrides
    ...
    for k, v in data_class.__dataclass_fields__.items():
        ...
        elif isinstance(val, TrevDataclass):
...
def override_module_args(args: Namespace) -> Tuple[List[str], List[str]]:
    ...
    for k in TrevConfig.__dataclass_fields__.keys():
        overrides.extend(
            _override_attr(k, TrevConfig.__dataclass_fields__[k].type, args)
        )
    if args is not None:
        if hasattr(args, "task"):
            from trev.tasks import TASK_DATACLASS_REGISTRY
        ...
        from trev.registry import REGISTRIES
        ...
        if hasattr(args, "arch"):
            from trev.models import ARCH_MODEL_REGISTRY, ARCH_MODEL_NAME_REGISTRY
...
def convert_namespace_to_omegaconf(args: Namespace) -> DictConfig:
    ...
    # configs will be in trev/config after installation
    ...
    if cfg.lr_scheduler is None and getattr(args, "lr_scheduler", None):
        ...
        from trev.optim.lr_scheduler import LR_SCHEDULER_REGISTRY
    ...
    if cfg.criterion is None and getattr(args, "criterion", None):
        ...
        from trev.criterions import CRITERION_REGISTRY
...
def merge_with_parent(dc: TrevDataclass, cfg: TrevDataclass):

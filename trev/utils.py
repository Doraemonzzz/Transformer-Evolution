...
from trev.modules.multihead_attention import MultiheadAttention
...
def load_ensemble_for_inference(filenames, task, model_arg_overrides=None):
    from trev import checkpoint_utils
...
def import_user_module(args):
    ...
    if module_path is not None:
        ...
        if not os.path.exists(module_path) and not os.path.isfile(os.path.dirname(module_path)):
            trev_rel_path = os.path.join(os.path.dirname(__file__), args.user_dir)
            if os.path.exists(trev_rel_path):
                module_path = trev_rel_path
            else:
                trev_rel_path = os.path.join(
...
def get_perplexity(loss, round=2, base=2):
    from trev.logging.meters import safe_round
...
def get_activation_fn(activation: str) -> Callable:
    ...
    from trev.modules import gelu, gelu_accurate

...
from trev.dataclass.utils import merge_with_parent, populate_dataclass
from hydra.core.config_store import ConfigStore

from .composite_encoder import CompositeEncoder
from .distributed_trev_model import DistributedTrevModel
from .trev_decoder import TrevDecoder
from .trev_encoder import TrevEncoder
from .trev_incremental_decoder import TrevIncrementalDecoder
from .trev_model import (
    BaseTrevModel,
...
def build_model(cfg: TrevDataclass, task):
...
def register_model(name, dataclass=None):
    """
    New model types can be added to trev with the :func:`register_model`
    function decorator.

    For example::

        @register_model('lstm')
        class LSTM(TrevEncoderDecoderModel):
            (...)

    .. note:: All models must implement the :class:`BaseTrevModel` interface.
        Typically you will extend :class:`TrevEncoderDecoderModel` for
        sequence-to-sequence tasks or :class:`TrevLanguageModel` for
    ...
    def register_model_cls(cls):
        ...
        if not issubclass(cls, BaseTrevModel):
            raise ValueError(
                "Model ({}: {}) must extend BaseTrevModel".format(name, cls.__name__)
            )
        MODEL_REGISTRY[name] = cls
        if dataclass is not None and not issubclass(dataclass, TrevDataclass):
            raise ValueError(
                "Dataclass {} must extend TrevDataclass".format(dataclass)
            )
        ...
        if dataclass is not None:
            ...
            node = dataclass()
            node._name = name
            cs.store(name=name, group="model", node=node, provider="trev")
...
def register_model_architecture(model_name, arch_name):
    """
    New model architectures can be added to trev with the
...
for file in os.listdir(models_dir):
    ...
    if (
        ...
        module = importlib.import_module("trev.models." + model_name)

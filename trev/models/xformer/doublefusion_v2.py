...
from trev.modules import DoubleFusionV2EncoderLayer, DoubleFusionV2DecoderLayer

class DoubleFusionV2Encoder(TransformerEncoder):
    """
    ...
    def build_encoder_layer(self, args):
        layer = DoubleFusionV2EncoderLayer(args)
...
class DoubleFusionV2Decoder(TransformerDecoder):
    def __init__(
        self,
        args,
        dictionary,
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        layer = DoubleFusionV2DecoderLayer(args, no_encoder_attn)
        checkpoint = getattr(args, "checkpoint_activations", False)
        if checkpoint:
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
...
@register_model("double_fusion_v2_model")
class DoubleFusionV2Model(TransformerModel):
    """
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return DoubleFusionV2Encoder(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return DoubleFusionV2Decoder(
            args,
            tgt_dict,
            embed_tokens,

...
from trev.modules import RFAEncoderLayer, RFADecoderLayer
...
class RFAEncoder(VanillaTransformerEncoder):
    """
    RFA encoder consisting of *args.encoder_layers* layers. Each layer
    is a :class:`RFAEncoderLayer`.
...
class RFADecoder(VanillaTransformerDecoder):
    """
    RFA decoder consisting of *args.decoder_layers* layers. Each layer
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        layer = RFADecoderLayer(args, no_encoder_attn)
        checkpoint = getattr(args, "checkpoint_activations", False)
        if checkpoint:
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)

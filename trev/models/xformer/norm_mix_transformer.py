...
from trev.modules import NormMixAttentionEncoderLayer, NormMixAttentionDecoderLayer
...
class NormMixAttentionEncoder(TransformerEncoder):
    ...
    def build_encoder_layer(self, args):
        layer = NormMixAttentionEncoderLayer(args)
...
class NormMixAttentionDecoder(TransformerDecoder):
    def __init__(
        self,
        args,
        dictionary,
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        layer = NormMixAttentionDecoderLayer(args, no_encoder_attn)
        checkpoint = getattr(args, "checkpoint_activations", False)
        if checkpoint:
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)

...
from trev.modules import DoubleFusionV3EncoderLayer, DoubleFusionV3DecoderLayer

class DoubleFusionV3Encoder(TransformerEncoder):
    """
    ...
    def build_encoder_layer(self, args):
        layer = DoubleFusionV3EncoderLayer(args)
...
class DoubleFusionV3Decoder(TransformerDecoder):
    def __init__(
        self,
        args,
        dictionary,
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        layer = DoubleFusionV3DecoderLayer(args, no_encoder_attn)
        checkpoint = getattr(args, "checkpoint_activations", False)
        if checkpoint:
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
...
@register_model("double_fusion_v3_model")
class DoubleFusionV3Model(TransformerModel):
    """
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return DoubleFusionV3Encoder(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return DoubleFusionV3Decoder(
            args,
            tgt_dict,
            embed_tokens,

...
from trev.modules import VanillaTransformerEncoderLayer, VanillaTransformerDecoderLayer
...
class VanillaTransformerEncoder(TrevEncoder):
    """
    VanillaTransformer encoder consisting of *args.encoder_layers* layers. Each layer
    is a :class:`VanillaTransformerEncoderLayer`.
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if getattr(args, "layernorm_embedding", False):
            embedding_norm_type = getattr(args, 'embdding_layernorm', 'layernorm')
            logging.info(f"Encoder embedding layernorm type: {embedding_norm_type}")
            self.layernorm_embedding = get_norm(embedding_norm_type, embed_dim)
        else:
        ...
        logging.info(f"Encoder attention types: {attention_types}")
    ...
    def build_encoder_layer(self, args):
        layer = VanillaTransformerEncoderLayer(args)
...
class VanillaTransformerDecoder(TrevIncrementalDecoder):
    """
    VanillaTransformer decoder consisting of *args.decoder_layers* layers. Each layer
    is a :class:`VanillaTransformerDecoderLayer`.
    ...
    def __init__(
        ...
        if getattr(args, "layernorm_embedding", False):
            embedding_norm_type = getattr(args, 'embdding_layernorm', 'layernorm')
            logging.info(f"Decoder embedding layernorm type: {embedding_norm_type}")
            self.layernorm_embedding = get_norm(embedding_norm_type, embed_dim)
        else:
            self.layernorm_embedding = None
        ...
        if attention_types == []:
            attention_types = getattr(args, "attention_types", [])
        logging.info(f"Decoder attention types: {attention_types}")
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        layer = VanillaTransformerDecoderLayer(args, no_encoder_attn)
        checkpoint = getattr(args, "checkpoint_activations", False)
        if checkpoint:
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
    ...
    def extract_features_scriptable(
        ...
        Translate with VanillaTransformer Models" (Garg et al., EMNLP 2019).

...
from trev.modules import ToeplitzAttentionEncoderLayer, ToeplitzAttentionDecoderLayer

class ToeplitzAttentionEncoder(TransformerEncoder):
    """
    ...
    def build_encoder_layer(self, args):
        layer = ToeplitzAttentionEncoderLayer(args)
...
class ToeplitzAttentionDecoder(TransformerDecoder):
    def __init__(
        self,
        args,
        dictionary,
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        layer = ToeplitzAttentionDecoderLayer(args, no_encoder_attn)
        checkpoint = getattr(args, "checkpoint_activations", False)
        if checkpoint:
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
...
@register_model("ToeplitzAttention")
class ToeplitzModel(TransformerModel):
    """
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return ToeplitzAttentionEncoder(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return ToeplitzAttentionDecoder(
            args,
            tgt_dict,
            embed_tokens,

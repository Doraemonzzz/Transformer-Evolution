...
from trev.models.xformer import FlashQuadEncoder

class RobertaFlashQuadEncoder(RobertaEncoder):
    """RoBERTa encoder."""
    ...
    def build_encoder(self, args, dictionary, embed_tokens):
        encoder = FlashQuadEncoder(args, dictionary, embed_tokens)
        encoder.apply(init_bert_params)
        return encoder

@register_model("roberta_flash_quad")
class RobertaFlashModel(RobertaModel):
    ...
    def build_model(cls, args, task):
        ...
        encoder = RobertaFlashQuadEncoder(args, task.source_dictionary)
        return cls(args, encoder)

@register_model_architecture("roberta_flash_quad", "roberta_flash_quad_v1")
def roberta_base_architecture_flash_quad(args):
    base_architecture(args)

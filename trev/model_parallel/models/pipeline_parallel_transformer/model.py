...
from trev.model_parallel.models.pipeline_parallel_transformer.layers import (
    Embedding,
...
from trev.modules import SinusoidalPositionalEmbedding
...
class PipelineParallelTransformerModel(BaseTrevModel):
    def __init__(self, encoder, decoder, balance, devices, chunks, checkpoint):
        import_pipe()
        super().__init__()
        assert isinstance(encoder, TrevEncoder)
        assert isinstance(decoder, TrevDecoder)
        encoder_module_list = (
...
class TransformerDecoder(TrevDecoder):
    """

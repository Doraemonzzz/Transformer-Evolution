...
from trev.distributed import utils as dist_utils
...
@dataclass
class TruncatedBPTTLMConfig(TrevDataclass):
...
class TruncatedBPTTLMTask(TrevTask):
    ...
    def _collate_fn(self, items: List[List[torch.Tensor]]):
        # we don't use trev's batching functionality, so we expect a single
        ...
        # trev expects batches to have the following structure
        return {
